# AuTransactional BFF — Guía de APIs y pruebas en Postman

Guía de todos los endpoints expuestos por el BFF y de la API de KiraFin que consume por
detrás, con el orden exacto en que hay que llamarlos para que el flujo funcione.

- **Base local:** `http://localhost:8080`
- **Swagger UI:** `http://localhost:8080/swagger-ui.html` — OpenAPI en `/v3/api-docs`
- **Colección Postman:** `docs/postman/AuTransactional.postman_collection.json`
- **Entorno Postman:** `docs/postman/AuTransactional-local.postman_environment.json`
- **Colección Bruno (incluye RFIs):** `docs/bruno/AuTransactional/` — guía paso a paso en [`GUIA-BRUNO.md`](GUIA-BRUNO.md)
...
| Perfil | `dev` (por defecto). Usa adaptadores biométricos locales deterministas, sin AWS |

### 1.2 Variables de entorno

En perfil `dev` sólo el webhook necesita secreto; el resto tiene valores por defecto locales.

```bash
export KIRA_WEBHOOK_SECRET='secreto-webhook-local'   # el mismo que pongas en Postman
# Opcionales: sólo si vas a aprobar pagos contra el sandbox de Kira
export KIRA_API_KEY='...'
export KIRA_CLIENT_ID='...'
...
y un operador por rol en cada una (3 × 5):

- Organizaciones (`clientId` / `tenantId`): `juriscop`, `bankvision`, `au-colombia`
...
### 1.4 Importar en Postman

1. **Import** → arrastra los dos ficheros de `docs/postman/`.
2. Selecciona el entorno *AuTransactional - local (perfil dev)*.
3. Ajusta `webhookSecret` para que coincida con `KIRA_WEBHOOK_SECRET`.
4. Para los endpoints con ficheros: **Settings → General → Working directory** apuntando a
   `docs/postman/`, o vuelve a seleccionar el fichero a mano en cada petición.

La colección guarda sola los tokens e identificadores: ejecuta las carpetas en orden y las
variables se van encadenando.
...
|---|---|
| `/api/auth/me`, `/api/payouts/**` | JWT propio del BFF (`Authorization: Bearer <token>`), HS256, 8 h |
| `/api/webhooks/kira` | HMAC-SHA256 en la cabecera `x-signature-sha256` |
| `/api/v1/liveness/**`, `/api/v1/number-challenge/**`, `/api/v1/identity/validate` | Públicos. El aislamiento por organización lo da el `clientId` |
...
| Error de identidad | 401 / 404 / 422 / 500 | el nombre del `IdentityErrorCode` |
| Error de Kira | 422, o 502 si Kira dio 5xx/401 | `kira_<codigo>` |
| No controlado — incluye **arrancar sin `KIRA_API_KEY`** y llamar a algo que use Kira | 500 | `internal_error` |
...
Historial de fondeos. **Sólo lectura**: los depósitos no se crean desde aquí, se proyectan
desde los webhooks.
...
Validaciones: `virtualAccountId`, `recipientId` y `currency` no vacíos; `amount` ≥
`0.00000001`. Respuesta `201` con la proyección `PayoutView`:
...
### 4.9 Verificación de identidad *(pública)*

Tres pasos que **deben pertenecer a la misma sesión de verificación** y ejecutarse en este
orden. El dominio lo impone: la prueba de vida sólo se ofrece a quien ya superó el reto de
voz, y el veredicto sólo se emite si ambas están superadas.

```
POST /api/v1/number-challenge/session      -> verificationId + challengeId + challengeNumber
POST /api/v1/number-challenge/{id}/verify  -> passed:true
POST /api/v1/liveness/session              -> livenessSessionId  (¡con verificationId!)
GET  /api/v1/liveness/session/{id}/result  -> passed:true + referenceImageBase64
POST /api/v1/identity/validate             -> veredicto del servidor
```

La sesión de verificación caduca a los 1800 s; el reto de voz, a los 120 s.

#### 4.9.1 Reto de voz — `/api/v1/number-challenge`

**`POST /session`** — query `clientId` (obligatorio si no mandas `verificationId`),
`verificationId` (para reutilizar sesión), cabecera opcional `X-Correlation-Id`.

```json
{ "challengeId": "b7...", "challengeNumber": "4821",
  "expiresAt": "2026-09-08T18:02:00Z", "verificationId": "3a..." }
```

Un `clientId` que no corresponda a una organización activa → `401 UNAUTHORIZED`.

**`POST /{challengeId}/verify`** — `multipart/form-data` con la parte `recording`
(vídeo + audio juntos a propósito: el número se valida sobre el audio y el movimiento
labial sobre el vídeo).

En perfil `dev` no hay transcripción real: `DevSpeechTranscriber` busca los primeros
4 dígitos en los **primeros 4096 bytes** del fichero, y considera que **no** hubo
movimiento labial si encuentra la cadena `NOLIPS`. Genera el fichero así:

```bash
cd docs/postman
./generar-grabacion.sh 4821            # camino feliz, usa el challengeNumber que te devolvió
./generar-grabacion.sh 4821 --nolips   # simula un vídeo pregrabado
```

Respuesta (siempre `200`, incluso al fallar, para que el frontend pueda reintentar):

```json
{ "passed": true, "transcribedText": "el numero es 4821", "normalizedNumber": "4821",
  "confidence": 0.94, "failureReason": null }
```

`failureReason` posibles: `INVALID_SPOKEN_NUMBER`, `LIP_MOVEMENT_NOT_DETECTED`,
`AUDIO_TRANSCRIPTION_FAILED`, `NUMBER_CHALLENGE_EXPIRED`, `NUMBER_CHALLENGE_ALREADY_USED`,
`MAX_ATTEMPTS_REACHED`.

El reto es de **un solo uso** y admite **3 intentos**; cada intento fallido se persiste.

**`GET /{challengeId}/status`** → `{ "valid": false, "used": true, "expired": false }`.

#### 4.9.2 Prueba de vida — `/api/v1/liveness`

| Endpoint | Devuelve |
|---|---|
| `GET /config` | `{ enabled, identityPoolId, region }` — datos públicos, nunca credenciales |
| `GET /status` | `{ enabled }` |
| `POST /session` | `{ sessionId, createdAt, expiresAt, verificationId }` |
| `GET /session/{sessionId}/result` | `{ sessionId, status, confidence, passed, referenceImageBase64 }` |

> **El error más común:** llamar a `POST /session` con `clientId` en vez de
> `verificationId`. Eso crea una sesión nueva sin reto de voz superado y responde
> `422 VERIFICATION_INCOMPLETE`. Pasa siempre el `verificationId` que te devolvió el reto.

En perfil `dev` el resultado es determinista (`bff.identity.dev-liveness-passes`, por
defecto `true`): `SUCCEEDED` con `confidence: 96.5` y un `referenceImageBase64` de 1 píxel.
Ponlo a `false` para recorrer el camino de rechazo. `status` puede ser `SUCCEEDED`,
`FAILED`, `EXPIRED` o `NOT_FOUND`.

#### 4.9.3 Veredicto — `POST /api/v1/identity/validate`

`multipart/form-data`. Parámetros de query:

| Parámetro | Obligatorio | Nota |
|---|---|---|
| `verificationId` | sí | |
| `challengeId` | sí | ata el reto de voz a esta sesión |
| `livenessSessionId` | sí | ata la prueba de vida a esta sesión |
| `countryCode` | sí | p. ej. `CO` |
| `documentType` | sí | p. ej. `NATIONAL_ID` |
| `kiraUserId` | no | |

Partes del cuerpo: `selfieImage` (opcional en el contrato, pero necesaria para un veredicto
útil — debe ser el `referenceImageBase64` de la prueba de vida, no una foto aparte),
`documentFrontImage` (obligatoria), `documentBackImage` (opcional). Máximo 10 MB por
fichero, 32 MB por petición.

```json
{
  "verificationId": "3a...", "status": "APPROVED",
  "livenessScore": 96.5, "matchScore": 92.0,
  "documentData": { "documentNumber": "1020304050", "firstName": "MARIA",
                    "lastName": "GONZALEZ", "birthDate": "1990-05-15",
                    "expirationDate": "2030-05-15", "issuingCountry": "CO",
                    "nationality": "CO", "gender": "F" },
  "failureCode": null
}
```

**El veredicto lo emite el servidor, nunca el navegador.** Umbrales (configurables en
`bff.identity`): `min-liveness-score: 80`, `min-match-score: 85`, `review-floor: 70`.

| Condición | Veredicto |
|---|---|
| `livenessScore` < 80 | `REJECTED` (`failureCode: LIVENESS_FAILED`) |
| `matchScore` ≥ 85 | `APPROVED` |
| 70 ≤ `matchScore` < 85 | `MANUAL_REVIEW` |
| `matchScore` < 70 | `REJECTED` (`failureCode: SPOOFING_DETECTED`) |
| 0 caras en el documento | `REJECTED` (`DOCUMENT_UNREADABLE`) |
| > 1 cara | `REJECTED` (`MULTIPLE_FACES`) |

En perfil `dev`, `DevFaceComparator` devuelve una similitud determinista entre 88 y 97
cuando ambas imágenes tienen al menos **64 bytes** — por eso los ficheros de muestra de
`docs/postman/muestras/` son PNG de 70 bytes y no ficheros vacíos. Si el documento viene
vacío el score es 0 y el veredicto, `REJECTED`.

Las imágenes **no se persisten**: sólo veredicto, puntajes y datos del OCR.

Casos de error para probar:

| Qué haces | Respuesta |
|---|---|
| `challengeId` de otra sesión | `422 SESSION_MISMATCH` |
| Sin haber superado voz o liveness | `422 VERIFICATION_INCOMPLETE` |
| `verificationId` inexistente o caducado | `404 SESSION_NOT_FOUND` |
| Repetir validate sobre una sesión ya resuelta | `404 SESSION_NOT_FOUND` ("ya fue resuelta") |
| Sin `documentFrontImage` | `422 DOCUMENT_UNREADABLE` |

---

### 4.10 Webhooks de Kira — `POST /api/webhooks/kira` *(HMAC)*
...
**Firmar desde Postman** — el pre-request script de la colección ya lo hace:
...
El cuerpo de la petición es literalmente `{{webhookBody}}`, para firmar exactamente la
misma cadena que se envía.
...
`GET /actuator/health` está permitido en la seguridad y configurado en `application.yaml`, pero
**hoy responde `500 internal_error`**: el `pom.xml` no incluye `spring-boot-starter-actuator`,
así que la ruta no existe. Para comprobar que la app está viva usa `/swagger-ui.html`.

---

### 4.12 Solicitudes de información (RFI) — `/api/rfis` *(JWT)*
...
| `items` | tal cual los entrega Kira: el formulario se dibuja desde `answer_type` y `answer_spec` |
| `blocking` | lo detenido. `payoutId`/`payoutStatus` sólo si el pago es de este portal y de esta empresa |
...
por el pago bloqueado—, aunque Kira la devuelva: el aislamiento lo imponemos nosotros. Si una
...
La subida de documentos (`POST /v1/rfis/{id}/items/{item_id}/documents`) **no está
...
| Método | Ruta de Kira | Uso |
|---|---|---|
| `GET`/`POST`/`PUT` | `/v1/users`, `/v1/users/{id}` | usuarios |
| `POST` | `/v1/users/{id}/liveness-link` | liveness alojada de Kira (KYB de UBO) |
| `GET`/`POST` | `/v1/virtual-accounts`, `/v1/virtual-accounts/{id}` | cuentas virtuales |
| `GET` | `/v1/virtual-accounts/{id}/balance` | saldo |
| `GET` | `/v1/virtual-accounts/deposits`, `/v1/virtual-accounts/{id}/deposits` | depósitos |
| `GET`/`POST` | `/v1/recipients`, `/v1/recipients/{id}` | beneficiarios |
| `POST` | `/v1/quotations` | cotizaciones |
| `POST` | `/v1/virtual-accounts/{id}/payout/preview` | previsualización |
| `POST` | **`/v1/virtual-accounts/{id}/payout`** | ← lo dispara `approve` |
| `GET` | **`/v1/payouts/{id}`**, `/v1/payouts` | ← lo dispara `refresh` |
| `GET`/`PATCH` | `/v1/rfis`, `/v1/rfis/{id}`, `/v1/rfis/{id}/items` | RFIs — **siempre con `X-Api-Version: 2026-06-01`** |
| `GET` | `/v1/countries` | catálogo |
...
de RFI**, que sólo existen en `2026-06-01` y la sobrescriben por petición
(`KiraApiClient.RFI_API_VERSION`).

---

## 6. Recorrido completo con curl
...
# --- Pagos ---
MAKER=$(curl -sS -X POST $BASE/api/auth/login -H 'Content-Type: application/json' \
  -d '{"email":"treasury.maker@juriscop.test","password":"Dev12345!"}' | jq -r .accessToken)
APPROVER=$(curl -sS -X POST $BASE/api/auth/login -H 'Content-Type: application/json' \
  -d '{"email":"treasury.approver@juriscop.test","password":"Dev12345!"}' | jq -r .accessToken)

PAYOUT=$(curl -sS -X POST $BASE/api/payouts -H "Authorization: Bearer $MAKER" \
  -H 'Content-Type: application/json' \
  -d '{"virtualAccountId":"va_demo_001","recipientId":"rec_demo_001","amount":"125.50","currency":"USD"}' \
  | jq -r .id)

curl -sS -X POST $BASE/api/payouts/$PAYOUT/reject -H "Authorization: Bearer $APPROVER" \
  -H 'Content-Type: application/json' -d '{"reason":"Prueba local."}' | jq

# --- Identidad ---
CH=$(curl -sS -X POST "$BASE/api/v1/number-challenge/session?clientId=$CLIENT")
VID=$(echo "$CH" | jq -r .verificationId); CID=$(echo "$CH" | jq -r .challengeId)
NUM=$(echo "$CH" | jq -r .challengeNumber)

printf 'grabacion de prueba numero=%s\n' "$NUM" > /tmp/recording.txt
curl -sS -X POST "$BASE/api/v1/number-challenge/$CID/verify" -F "recording=@/tmp/recording.txt" | jq

LS=$(curl -sS -X POST "$BASE/api/v1/liveness/session?verificationId=$VID" | jq -r .sessionId)
curl -sS "$BASE/api/v1/liveness/session/$LS/result" | jq

curl -sS -X POST "$BASE/api/v1/identity/validate?verificationId=$VID&challengeId=$CID&livenessSessionId=$LS&countryCode=CO&documentType=NATIONAL_ID" \
  -F "selfieImage=@docs/postman/muestras/selfie.png" \
  -F "documentFrontImage=@docs/postman/muestras/documento-frente.png" | jq
```
...
| `500 internal_error` al llamar a Kira | arrancaste sin `KIRA_API_KEY` (el log dice `Falta KIRA_API_KEY`) |
| `500` en `/actuator/health` | falta la dependencia de Actuator en el `pom.xml` |
| `403 forbidden` al crear un pago | estás usando el token del approver |
| `422 VERIFICATION_INCOMPLETE` en `liveness/session` | mandaste `clientId` en vez de `verificationId` |
| `422 SESSION_MISMATCH` en `validate` | mezclaste ids de sesiones distintas |
| `passed:false` con `INVALID_SPOKEN_NUMBER` | el fichero no contiene el `challengeNumber` de *este* reto |
| `404 SESSION_NOT_FOUND` | la sesión caducó (1800 s) o ya se emitió veredicto |
| `503 webhook_secret_not_configured` | falta `KIRA_WEBHOOK_SECRET` en el arranque |
| `401 invalid_signature` | Postman reformateó el JSON: firma la misma cadena que envías |
| `502 kira_*` al aprobar | Kira rechazó la llamada; la aprobación se revierte |
| Postman no encuentra el fichero | fija el *working directory* en `docs/postman/` |
